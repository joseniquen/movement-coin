...
import com.example.monederomovementmicroservice.aplication.MovementOperations;
...
import com.example.monederomovementmicroservice.utils.*;
...
public class MovementOperationsImpl implements MovementOperations {
    Logger logger = LoggerFactory.getLogger(MovementOperationsImpl.class);
    ...
    private final CoinOperations coinOperations;
    ...
    public Mono<ResponseService> create(Movement movement) {
        return validateDataWalletToCreate(movement).flatMap(RS -> {
            ...
            if (responseService.getStatus() == Status.OK) {
                return coinOperations.get(movement.getProduct()).flatMap(walletR -> {
    ...
    public Mono<ResponseService> errorNotExistWallet(Movement movement) {
        responseService = new ResponseService();
        responseService.setStatus(Status.ERROR);
        responseService.setMessage("No existe el monedero " + movement.getProduct());
